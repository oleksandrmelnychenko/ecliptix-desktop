...
public static class SodiumInterop
{
    ...
    static SodiumInterop()
    {
        try
        {
            if (sodium_init() < 0) throw new InvalidOperationException("sodium_init() returned an error code.");
    ...
    {
        return Result<byte[], ShieldFailure>.FromValue(buffer, ShieldFailure.InvalidInput("Buffer cannot be null."))
