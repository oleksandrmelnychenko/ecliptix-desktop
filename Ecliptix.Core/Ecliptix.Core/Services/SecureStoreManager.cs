...
public class SecureStoreManager : IDisposable
{
    ...
    public SecureStoreManager(string storeFilePath, string keyOrPassword, StoreLoadMethod loadMethod)
    {
        ...
        // Then load the key into this instance
        switch (loadMethod)
        {
            case StoreLoadMethod.ByKeyFile:
                if (!File.Exists(keyOrPassword))
                    throw new FileNotFoundException($"Key file not found: {keyOrPassword}");
                _secretsManager.LoadKeyFromFile(keyOrPassword);
                break;
            case StoreLoadMethod.ByPassword:
                if (string.IsNullOrEmpty(keyOrPassword))
                    throw new ArgumentNullException(nameof(keyOrPassword), "Password cannot be null or empty.");
                _secretsManager.LoadKeyFromPassword(keyOrPassword);
                break;
            default:
    ...
    public void Set(string key, byte[] value)
    {
        ...
        if (value == null)
        {
            // Store a special marker to indicate null value
            _secretsManager.Set(key, "NULL_VALUE");
            return;
        }
