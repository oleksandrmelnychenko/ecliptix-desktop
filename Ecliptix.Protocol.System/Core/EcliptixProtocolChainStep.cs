            ...
            .Bind(dhInfo =>
                AllocateAndWriteChainKey(initialChainKey)
                    .Bind(chainKeyHandle =>
                    {
                        ...
                        {
                            Console.WriteLine($"[EcliptixProtocolChainStep] Created Chain Key: {Convert.ToHexString(chainKeyTemp)}");
                        }
    ...
    internal Result<EcliptixMessageKey, EcliptixProtocolFailure> GetOrDeriveKeyFor(uint targetIndex)
    {
        ...
        try
        {
            ...
            for (uint idx = currentIndex + 1; idx <= targetIndex; idx++)
            {
                ...
                {
                    Console.WriteLine($"[EcliptixProtocolChainStep] Updated Chain Key at index {idx}: {Convert.ToHexString(chainKeyTemp)}");
                }
            }
