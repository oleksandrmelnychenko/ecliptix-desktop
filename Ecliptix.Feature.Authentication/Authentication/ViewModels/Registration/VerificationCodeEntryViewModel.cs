...
public sealed partial class VerificationCodeEntryViewModel : Core.Core.MVVM.ViewModelBase, IRoutableViewModel, IResettable
{
    ...
    [Reactive] public string TimerHintText { get; private set; } = string.Empty;
    ...
    private CountdownUpdateStatus CurrentStatus { get; set; } =
        CountdownUpdateStatus.CountdownActive;
    ...
    private void HandleResendError(string error)
    {
        RxApp.MainThreadScheduler.Schedule(() =>
        {
            ...
            else
            {
                PublishError(error);
                SecondsRemaining = 0;
                CurrentStatus = CountdownUpdateStatus.CountdownExpired;
            }
        });
    }
    ...
    private async Task StartAutoRedirectAsync(int seconds, MembershipViewType targetView, string localizedMessage = "")
    {
        ...
        else if (CurrentStatus == CountdownUpdateStatus.CountdownSessionExpired)
        {
        ...
        else if (CurrentStatus == CountdownUpdateStatus.CountdownNotFound)
        {
        ...
        bool alreadyVerified)
    {
        ...
        bool isMaxAttemptsKey = normalizedMessageKey != null &&
                                IsMessageKey(normalizedMessageKey, VerificationMessageKeys.OtpMaxAttemptsReached);
        bool isRateLimitKey = IsRateLimitKey(normalizedMessageKey);

        if (_initialTotalSeconds == null && seconds > 0 && status == CountdownUpdateStatus.CountdownActive)
        {
        ...
        {
            if (IsMessageKey(normalizedMessageKey, VerificationMessageKeys.VerificationFlowExpired))
            {
                status = CountdownUpdateStatus.CountdownSessionExpired;
            }
            else if (IsMessageKey(normalizedMessageKey, VerificationMessageKeys.OtpExpired))
            {
                status = CountdownUpdateStatus.CountdownExpired;
            }
            else if (IsMessageKey(normalizedMessageKey, VerificationMessageKeys.ResendCooldown))
            {
                status = CountdownUpdateStatus.CountdownResendCooldown;
            }
        }
        ...
        if (status == CountdownUpdateStatus.CountdownFailed &&
        ...
        if (status == CountdownUpdateStatus.CountdownActive && seconds == 0)
        {
            status = CountdownUpdateStatus.CountdownExpired;
        }
        ...
        {
            IsMaxAttemptsReached = status == CountdownUpdateStatus.CountdownMaxAttemptsReached;
        }
        ...
        string? message)
    {
        ...
        {
            CountdownUpdateStatus.CountdownActive => seconds,
            CountdownUpdateStatus.CountdownExpired => HandleExpiredStatus(message),
            CountdownUpdateStatus.CountdownResendCooldown => HandleResendCooldown(message,
                seconds),
            CountdownUpdateStatus.CountdownFailed => HandleFailedStatus(message),
            CountdownUpdateStatus.CountdownNotFound => HandleNotFoundStatus(),
            CountdownUpdateStatus.CountdownMaxAttemptsReached => HandleMaxAttemptsStatus(),
            CountdownUpdateStatus.CountdownSessionExpired => HandleSessionExpiredStatus(),
            CountdownUpdateStatus.CountdownServerUnavailable => HandleUnavailable(message),
    ...
    private static bool IsRateLimitKey(string? messageKey)
    {
        ...
        return IsMessageKey(messageKey, VerificationMessageKeys.SecurityRateLimitExceeded)
               || IsMessageKey(messageKey, VerificationMessageKeys.DeviceRateLimitExceeded)
               || IsMessageKey(messageKey, VerificationMessageKeys.MobileOtpLimitExhausted);
    }
    ...
    private string? ResolveCountdownMessage(string? message, string? messageKey)
    {
        ...
        if (IsMessageKey(messageKey, VerificationMessageKeys.OtpMaxAttemptsReached))
        {
        ...
        else if (IsMessageKey(messageKey, VerificationMessageKeys.OtpExpired) ||
                 IsMessageKey(messageKey, VerificationMessageKeys.VerificationFlowExpired))
        {
    ...
    private async Task ResetUiState()
    {
        ...
        await Dispatcher.UIThread.InvokeAsync(async () =>
        {
            ...
            CurrentStatus = CountdownUpdateStatus.CountdownActive;
            lock (_sessionLock)
            {
        ...
        bool isInNetworkOutage)
    {
        ...
        if (status == CountdownUpdateStatus.CountdownResendCooldown)
        {
        ...
        return status == CountdownUpdateStatus.CountdownExpired;
    }
}
