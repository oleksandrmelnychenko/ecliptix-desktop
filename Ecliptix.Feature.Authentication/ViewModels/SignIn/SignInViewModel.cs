...
namespace Ecliptix.Feature.Authentication.ViewModels.SignIn;
...
public sealed partial class SignInViewModel : Core.MVVM.ViewModelBase, IRoutableViewModel, IResettable, IDisposable
{
    ...
    public ReactiveCommand<SystemU, SystemU> OpenCountryPickerCommand { get; private set; } = null!;
    ...
    private void SetupCommands(IObservable<bool> isFormLogicallyValid)
    {
        ...
        SignInCommand = ReactiveCommand.CreateFromTask(
            async () =>
            {
                string fullNumber = MobileNumberHelper.CombineWithPrefix(PhonePrefix, MobileNumber);
                ...
                Result<Unit, AuthenticationFailure> result =
                    await _authRepository.SignInAsync(fullNumber, _secureKeyBuffer, connectId, cancellationToken);
                return result;
            },
            canSignIn);
        ...
        OpenCountryPickerCommand = ReactiveCommand.CreateFromTask(async () =>
        {
            await GlobalModalService.ShowRightAsync(
                new CountryCodeViewModel(_messageBus, LocalizationService, GlobalModalService, CountryIso, "SignIn"),
                showScrim: true,
                isDismissable: true
            );
        });
